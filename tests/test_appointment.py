...
class TestHome(Base):
    ...
    def test_loginStatus(self):
        ...
        log.info("Appointment.py button clicked")
        time.sleep(5)
    ...
    def test_appointment_details(self):
        ...
        log.info("check weather Appointment.py was book successfully or not")
        try:
            confirmation_text = self.driver.find_element(By.XPATH, "//div[@class='col-xs-12 text-center']/h2").text
            assert "Appointment.py Confirmation" in confirmation_text, log.error("Appointment.py unsuccessful")
        except AssertionError as ae:
            assert False, log.error(f"Assertion Error: {ae}")
        except Exception as exc:
            log.error(f"Exception error: {exc}")
